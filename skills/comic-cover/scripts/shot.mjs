#!/usr/bin/env node
// Screenshot a comic cover (or the model probe) in headless Chrome with the GPU on, run JS between frames,
// tile the frames into one contact sheet, and print the page's console output. No npm deps: Node 22+, Chrome, ffmpeg.
//
//   node shot.mjs <cover-dir> <out.png> [frameJS ...]
//     each frameJS runs in the page before its capture ('' = just capture). Return values are printed.
//     e.g.  node shot.mjs ./my-cover /tmp/s.png '' 'cover.clickAt(.8,.35)' 'cover.setStage(1)'
//
//   env: PAGE=index.html (may carry a ?query)  SIZE=900x1300  DPR=1  WAIT=450 (ms after each frameJS)
//        FULL=1 (whole viewport, not just the cover)  COLS=3  FPS=1 (print frames/sec)  CHROME=/path/to/chrome
...
if (!dir || !out) { console.error('usage: node shot.mjs <cover-dir> <out.png> [frameJS ...]'); process.exit(1); }
...
const tmp = mkdtempSync(join(tmpdir(), 'comic-shot-'));
